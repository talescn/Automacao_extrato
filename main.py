...
def check_for_stop():
    """Verifica se a tecla 'q' foi pressionada para parar o script."""
...
def navegar_para_baixa_entradas():
    ...
    check_for_stop()  # Verifica se a execução deve ser interrompida   
...
def automatizar_baixa(pdf_path):
    ...
    if valor_parcela:
        ...
        if verificar_pagamento(valor_parcela, valores_extrato):
            ...
            messagebox.showinfo("Aviso", "Nenhum pagamento correspondente encontrado.")
            root.destroy()
    else:
        print("Erro ao capturar valor da parcela.")
